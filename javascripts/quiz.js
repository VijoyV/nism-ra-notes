(function () {
    ...
    apps.forEach((app) => {
        ...
        app.innerHTML = `
            <div class="quiz-config-bar">
                <div class="quiz-config-grid">
                    ...
                    <div class="quiz-field">
                        <label for="${app.id}-time">Time</label>
                    ...
                    <button class="quiz-start-button" type="button">Start Exam</button>
                </div>
            </div>
            ...
            <div class="timer-line" hidden></div>
            <div class="quiz-wrapper">
                <div class="quiz-empty-state">Select number of questions and duration, then click "Start Exam" to begin.</div>
            </div>

            <div class="quiz-nav-controls" hidden>
            ...
            <div class="submit-container" hidden>
                <button class="quiz-submit-button" type="button">Submit Final Answers</button>
            </div>
        `;
        ...
        const els = {
            ...
            timer: app.querySelector(".timer-line"),
            wrapper: app.querySelector(".quiz-wrapper"),
            nav: app.querySelector(".quiz-nav-controls"),
            prev: app.querySelector(".btn-prev"),
            next: app.querySelector(".btn-next"),
            counter: app.querySelector(".question-counter"),
            submitWrap: app.querySelector(".submit-container"),
            submit: app.querySelector(".quiz-submit-button"),
        ...
        function startTimer() {
            ...
            els.timer.innerText = "Time Elapsed: " + formatTime(state.secondsElapsed);
        ...
        function showQuestion(index) {
            ...
            els.wrapper.innerHTML = `
                <div class="q-card">
                    ...
                    <div class="quiz-topic"><small>Topic: ${item.topic || "General"}</small></div>
            ...
            els.wrapper.querySelectorAll("input[type='radio']").forEach((input) => {
                input.addEventListener("click", () => {
                    if (!state.submitted) state.selectedAnswers[index] = parseInt(input.value, 10);
                });
            });

            els.counter.innerText = `Q ${index + 1} / ${state.questions.length}`;
            els.prev.disabled = index === 0;
            els.next.disabled = index === state.questions.length - 1;
        }
        ...
        els.prev.addEventListener("click", () => changeQuestion(-1));
        els.next.addEventListener("click", () => changeQuestion(1));
